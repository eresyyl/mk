...
foreach( $collumns as $type => $name ): ?>
	...
	switch($type){
		...
		case 'spaces':
			...
			break;
		case 'categories':
			?><p class="em-tickets-categories">
				<label><?php echo $name; ?></label>
				<?php foreach($EM_Event->get_categories()->categories as $category): ?>
					<input type="checkbox" name="categories[]" value="<?php echo $category->id?>"><?php echo $category->slug; ?>
				<?php endforeach; ?>
			</p>
			<?php break;
		default:
