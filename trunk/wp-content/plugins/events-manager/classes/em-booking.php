...
class EM_Booking extends EM_Object{
	...
	var $fields = array(
		...
		'booking_categories' => array('name'=>'categories','type'=>'%s'),
	...
	function EM_Booking( $booking_data = false ){
		...
		global $wpdb;
	  	if( $booking_data !== false ){
			...
			$booking['booking_categories'] = (!empty($booking['booking_categories'])) ? maybe_unserialize($booking['booking_categories']):array();
	...
	function save($mail = true){
		...
		if( $this->can_manage() ){
			...
			$data['booking_categories'] = serialize($data['booking_categories']);
			//update or save
			if($this->booking_id != ''){
				$update = true;
	...
	function get_post( $override_availability = false ){
		...
		if( !empty($_REQUEST['categories']) && isset($_REQUEST['em_tickets']) && is_array($_REQUEST['em_tickets']) && ($_REQUEST['em_tickets'] || $override_availability) ){
			...
			$this->booking_categories = $_REQUEST['categories'];
