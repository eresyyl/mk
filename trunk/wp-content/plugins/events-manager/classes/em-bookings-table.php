...
class EM_Bookings_Table{
	 ...
	 * @var array
	 */
	public $cols = array('user_name','event_name','booking_spaces','booking_categories','booking_status','booking_price','actions');
	/**
	...
	function __construct($show_tickets = false){
		...
		$this->cols_template = apply_filters('em_bookings_table_cols_template', array(
			...
			'booking_categories'=>__('Categories','dbem'),
	...
	function get_row( $object, $csv = false ){
		...
		foreach($this->cols as $col){
			...
			elseif( $col == 'booking_categories'){
				$temp_cats = '';
				foreach($EM_Booking->$col as $cat_id){
					if( $temp_cats != ''){
						$temp_cats .= ', ';
					}
					$temp_cats .= get_term_by( 'id', absint( $cat_id ), 'event-categories' )->slug;;
				}
				$cols[] = $temp_cats;
				unset($temp_cats);
			}
